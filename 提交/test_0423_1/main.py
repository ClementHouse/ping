...
def data_process_train(path_train):
   ...
   # train_columns.append("month")
    ...
    train_data.drop(['TERMINALNO', 'TIME', 'TRIP_ID', 'CALLSTATE', 'hour'], axis=1, inplace=True)
    ...
    train_data.drop(['LONGITUDE','LATITUDE','DIRECTION','HEIGHT','SPEED','Y'],axis=1,inplace=True)
    train_data=pd.concat([train_data,train_data_float],axis=1)
    # print (train_data)
    print (mem_usage(train_data))
...
def data_process_test(path_test):
    ...
    test_data.drop(['TIME', 'TRIP_ID', 'CALLSTATE', 'hour'], axis=1, inplace=True)
    ...
    test_data.drop(['TERMINALNO','LONGITUDE', 'LATITUDE', 'DIRECTION', 'HEIGHT', 'SPEED'], axis=1, inplace=True)
    test_data = pd.concat([test_data, test_data_float,test_data_int], axis=1)
    # print (test_data)
    print(mem_usage(test_data))
